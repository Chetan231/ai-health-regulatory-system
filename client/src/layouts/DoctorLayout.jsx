...
import {
  FiActivity, FiHome, FiUsers, FiUser, FiCalendar, FiClock,
  FiFileText, FiMessageCircle, FiCpu, FiLogOut, FiMenu, FiX, FiBell
} from 'react-icons/fi';
...
const navItems = [
  ...
  { to: '/doctor/ai-assist', icon: FiCpu, label: 'AI Assist' },
  { to: '/doctor/chat', icon: FiMessageCircle, label: 'Chat' },
];
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] ml-1.5 bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-medium">Doctor</span>
              </div>
            </div>
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            {navItems.map((item) => (
              <NavLink key={item.to} to={item.to} onClick={() => setSidebarOpen(false)}
                ...
                {item.badge && <span className="ml-auto text-[10px] bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">{item.badge}</span>}
              </NavLink>
            ))}
          </nav>
          ...
          <div className="p-4 border-t border-gray-100">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">Dr. {user?.name}</p>
